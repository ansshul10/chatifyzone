...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await api.post('/auth/register', { username, email, password }); // Fixed endpoint
      localStorage.setItem('token', data.token);
      localStorage.setItem('user', JSON.stringify(data.user)); // Store user object for later use
      api.defaults.headers.common['x-auth-token'] = data.token; // Set token for authenticated requests
      setSuccess(true);
      setTimeout(() => navigate('/login'), 2000); // Redirect to chat after signup
    } catch (err) {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 flex flex-col lg:flex-row gap-8 lg:gap-12 flex-grow">
        ...
        >
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-center lg:text-left">
            Join the Chatify for Elite Experience
          </h1>
