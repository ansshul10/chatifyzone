...
const Home = () => {
  ...
  const testimonials = [
    {
      quote: 'Chatify transformed how I connect with friends. It’s fast, secure, and the anonymous mode is a game-changer!',
      author: 'Anonymous User',
    },
    {
      quote: 'The best messaging app I’ve used—intuitive design and lightning-fast performance!',
      author: 'Jane D.',
    },
    {
      quote: 'I love the privacy features and the ability to chat globally without hassle.',
      author: 'Mark S.',
    },
  ];
  ...
  const faqs = [
    {
      ...
      answer: 'Chatify is a modern messaging platform offering real-time, secure, and anonymous chat options.',
    },
    {
      question: 'Is my data safe?',
      answer: 'Yes, we use end-to-end encryption and strict privacy protocols to protect your data.',
    },
    {
      question: 'Can I use Chatify without signing up?',
      answer: 'Absolutely! Use our guest mode to chat anonymously without an account.',
    },
  ];
    ...
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 flex flex-col gap-16 flex-grow">
        ...
        <section className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start mt-6">
              <motion.div whileHover="hover" whileTap="tap" variants={buttonVariants}>
                <Link
                  to="/login"
                  className={`p-4 rounded-lg font-semibold shadow-lg flex items-center space-x-2 ${isDarkMode ? 'bg-[#1A1A1A] text-red-600' : 'bg-gray-300 text-red-500'}`}
                >
                  <FaUser />
                  <span>Login</span>
                </Link>
          ...
          {/* Enhanced "Launch Your Chat Now" with Tilt */}
          <motion.div
              ...
              <div className={`p-6 sm:p-6 rounded-xl shadow-2xl border ${isDarkMode ? 'bg-black border-gray-800' : 'bg-gray-200 border-gray-400'} hover:shadow-[10px_0_20px_rgba(255,0,0,0.3),-10px_0_20px_rgba(255,0,0,0.3)] transform transition-all duration-300`}>
                ...
                <ul className={`text-base sm:text-lg ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} space-y-4`}>
                  <li className="flex items-start space-x-2">
                    ...
                    <span>🎉 Instant chats with friends or strangers—your vibe, your rules!</span>
                  </li>
                  <li className="flex items-start space-x-2">
                    <FaCheckCircle className="text-red-500 mt-1" />
                    <span>🕵️‍♂️ Go incognito with anonymous mode—privacy guaranteed!</span>
                  </li>
                  <li className="flex items-start space-x-2">
                    <FaCheckCircle className="text-red-500 mt-1" />
                    <span>⚡ Lightning-fast messaging—no lag, just pure connection!</span>
                  </li>
                  <li className="flex items-start space-x-2">
                    <FaCheckCircle className="text-red-500 mt-1" />
                    <span>🌍 Global reach with a sleek, modern interface.</span>
                  </li>
                  <li className="flex items-start space-x-2">
                    <FaCheckCircle className="text-red-500 mt-1" />
                    <span>🎨 Customize your experience with themes and settings.</span>
                  </li>
                </ul>
              </div>
            </Tilt>
        ...
        <section className="w-full flex flex-col items-center space-y-12 py-12">
          <motion.h2
            variants={textVariants}
            className={`text-3xl sm:text-4xl font-extrabold text-center ${isDarkMode ? 'text-white' : 'text-gray-900'}`}
          >
            Why Choose Chatify? 🤔
          </motion.h2>
          ...
          <section className="w-full flex flex-col items-center space-y-8 py-12">
            ...
            <motion.p
              variants={textVariants}
              className={`text-base sm:text-lg ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} text-center max-w-xl`}
            >
        ...
        <section className={`w-full flex flex-col items-center space-y-12 py-12 rounded-xl p-6 ${isDarkMode ? 'bg-[#1A1A1A]' : 'bg-gray-200'}`}>
          <motion.h2
            ...
            className={`text-3xl sm:text-4xl font-extrabold text-center ${isDarkMode ? 'text-white' : 'text-gray-900'}`}
          >
            Chatify by the Numbers 📊
          </motion.h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
            <motion.div variants={cardVariants} className="text-center">
              <p className="text-4xl font-bold text-red-500">100K+</p>
              <p className={`mt-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Active Users</p>
            </motion.div>
            <motion.div variants={cardVariants} className="text-center">
              <p className="text-4xl font-bold text-red-500">1M+</p>
              <p className={`mt-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Messages Sent Daily</p>
            </motion.div>
            <motion.div variants={cardVariants} className="text-center">
              <p className="text-4xl font-bold text-red-500">99.9%</p>
              <p className={`mt-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Uptime Guarantee</p>
            </motion.div>
            <motion.div variants={cardVariants} className="text-center">
              <p className="text-4xl font-bold text-red-500">50+</p>
              <p className={`mt-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Countries Reached</p>
            </motion.div>
          </div>
        </section>
      </div>
